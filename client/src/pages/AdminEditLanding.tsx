...
function LandingPhotos({ landingId, images }: { landingId: string; images: LandingImage[] }) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [pendingCaptions, setPendingCaptions] = useState<Record<string, string>>({});

  const uploadMutation = useUploadLandingImage(landingId);
  const deleteMutation = useDeleteLandingImage(landingId);
  const captionMutation = useUpdateLandingImageCaption(landingId);
