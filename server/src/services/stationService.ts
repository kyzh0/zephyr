    ...
    for (const { station, data } of stationDataMap) {
      ...
      if (data.length) {
        ...
        if (timeNow - new Date(data[0].time).getTime() <= 60 * 60 * 1000) {
          ...
          for (const d of data) {
            if (d.windAverage != null || d.windGust != null) isWindError = false;
            if (d.windBearing != null) isBearingError = false;
            if (d.temperature != null) isTempError = false;
          }
        }
      }

      let errorMsg = '';
      if (isDataError) errorMsg = 'ERROR: Data scraper has stopped.\n';
      else if (isWindError) errorMsg += 'ERROR: No wind avg/gust data.\n';
      ...
      if (isDataError || isWindError || isBearingError || isTempError) {
        if (!station.isError) newErrorIds.push(station._id);
      }
    }
