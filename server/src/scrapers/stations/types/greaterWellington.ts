...
export default async function scrapeGreaterWellingtonData(stations: StationDoc[]): Promise<void> {
      ...
      limit(async () => {
        try {
          ...
          if (station.gwWindAverageFieldName) {
            ...
            if (data.length) windAverage = extractLastI1Number(data);
          }
          ...
          if (station.gwWindGustFieldName) {
            ...
            if (data.length) windGust = extractLastI1Number(data);
          }
          ...
          if (station.gwWindBearingFieldName) {
            ...
            if (data.length) windBearing = extractLastI1Number(data);
          }
          ...
          if (station.gwTemperatureFieldName) {
            ...
            if (data.length) temperature = extractLastI1Number(data);
          }
