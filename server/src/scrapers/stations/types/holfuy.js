  ...
  try {
    ...
    if (individualScrapeStations.length) {
      ...
      await Promise.allSettled(
        individualScrapeStations.map((station) =>
          limit(async () => {
            try {
              ...
              const { headers } = await httpClient.get(
                `https://holfuy.com/en/weather/${station.externalId}`
              );
              ...
              logger.warn(
                `An error occured while fetching data for holfuy - ${station.externalId}`,
                {
                  service: 'station',
                  type: 'holfuy'
                }
    ...
    logger.warn('An error occured while fetching data for holfuy', {
      service: 'station',
      type: 'holfuy'
    });
    logger.warn(error);
