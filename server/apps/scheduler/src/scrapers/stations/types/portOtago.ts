...
export default async function scrapePortOtagoData(stations: WithId<StationAttrs>[]): Promise<void> {
      ...
      limit(async () => {
        try {
          ...
          if (data.length) {
            ...
            if (avgKt != null) {
              windAverage = Math.round(avgKt * 1.852 * 100) / 100;
            }

            const gustKt = extractSeriesValue(data, 'Wind Gust Max');
            if (gustKt != null) {
              windGust = Math.round(gustKt * 1.852 * 100) / 100;
            }
